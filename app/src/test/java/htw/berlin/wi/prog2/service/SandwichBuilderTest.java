...
class SandwichBuilderTest {
    ...
    private final Ingredient loaf = new Ingredient("Brot", 0.02, 1000);

    @Test
    @DisplayName("can build a precomputed sandwich with two ingredients")
    void buildASandwich() {
        Sandwich sandwich = builder.add(loaf).add(kaese).buildPrecomputed();
    ...
    void buildTwoSandwiches() {
        Sandwich sandwich1 = builder.add(loaf).add(kaese).buildPrecomputed();
        Sandwich sandwich2 = builder.add(loaf).add(loaf).buildPrecomputed();
    ...
    void buildASandwichDynamically() {
        Sandwich sandwich = builder.add(loaf).add(kaese).buildDynamicallyComputed();
    ...
    void buildTwoSandwichesDynamically() {
        Sandwich sandwich1 = builder.add(loaf).add(kaese).buildDynamicallyComputed();
        Sandwich sandwich2 = builder.add(loaf).add(loaf).buildDynamicallyComputed();
    ...
    @Test
    @DisplayName("IllegalSandwichExceptionTest")
    void buildSandwich2(){

        assertThrows(IllegalSandwichException.class, () -> builder.add(loaf).buildDynamicallyComputed());
        assertThrows(IllegalSandwichException.class, () -> builder.add(loaf).buildPrecomputed());
    }
}